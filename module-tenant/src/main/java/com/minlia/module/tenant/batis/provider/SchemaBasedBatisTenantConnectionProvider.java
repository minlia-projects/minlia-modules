...
public class SchemaBasedBatisTenantConnectionProvider implements BatisTenantConnectionProvider {
  ...
  private static Pattern encodeStringPatterDatabaseName = Pattern
      .compile("databaseName=(.*?);", Pattern.CASE_INSENSITIVE);
  ...
  @Override
  public Connection getConnection(String tenantIdentifier, Connection connection)
      throws SQLException {
  ...
  private String databaseName(DatabaseMetaData databaseMetaData) {
    try {
      String url = databaseMetaData.getURL();
      int start = url.lastIndexOf("/") + 1;
      int end = url.indexOf("?");
      String databaseName = url.substring(start, end == -1 ? url.length() : end);
      return databaseName;
...
//            log.info("Default Schema {}", this.defaultSchema);
   ...
   * 应用租户schema
   */
  private void applyTenantSchema(String tenantIdentifier, Connection connection)
      throws SQLException {
    try {
      /**
