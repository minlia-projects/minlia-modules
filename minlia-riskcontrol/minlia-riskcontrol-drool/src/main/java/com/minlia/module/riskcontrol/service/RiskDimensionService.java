...
public class RiskDimensionService {
    ...
    private Long runSha(String key, Long minScore, Long maxScore, Long expire, Long score, String value) {
        Long result = redisTemplate.execute(longDefaultRedisScript, Collections.singletonList(key), minScore, maxScore, expire, score, value);
        return result;
    }
    ...
    private Long runSha1(String key, Long remMaxScore, Long expire, Long score, String value, Long queryMinScore, Long queryMaxScore) {
        Long result = redisTemplate.execute(longDefaultRedisScript, Collections.singletonList(key), remMaxScore, expire, score, value, queryMinScore, queryMaxScore);
        return result;
    }
    ...
    public long distinctCountWithRedisAndConfig(Event event, String[] condDimensions, String aggrDimension) {
        RiskDrools riskDrools = riskDroolsService.get(event.getScene());
        if (null == riskDrools) {
            event.setLevel(RiskLevelEnum.NORMAL);
            return 0;
        } else {
            long count = this.distinctCountWithRedis(event, condDimensions, riskDrools.getPeriodSeconds(), aggrDimension);
            event.addScore(count, riskDrools.getDangerThreshold(), riskDrools.getWarningThreshold(), riskDrools.getThresholdScore(), riskDrools.getPerScore());
            event.setDetails(String.format(riskDrools.getDetail(), count));
            return count;
        }
    }
    ...
    public void cleanCountWithRedis(Event event, String[] condDimensions, String aggrDimension) {
        ...
        redisTemplate.opsForZSet().removeRangeByScore(key, -1L, event.getTime());
    }
