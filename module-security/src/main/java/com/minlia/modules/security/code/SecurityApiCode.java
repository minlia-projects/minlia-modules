...
public class SecurityApiCode extends ApiCode {
  ...
  @Localized(values = {
      @Localize(type = LanguageTypes.ExceptionsApiCode, locale = "en_US", message = "No account bound to system user, please check that login with system admin, without bound to any account?"),
      @Localize(type = LanguageTypes.ExceptionsApiCode, locale = "zh_CN", message = "当前登录的用户未绑定系统账号, 是否使用了开发时自带的admin来测试功能?"),
  })
  public static final int ACCOUNT_NOT_BOUND_TO_USER = SECURITY_MODULE_CODE_BASED + 1;
  @Localized(values = {
      @Localize(type = LanguageTypes.ExceptionsApiCode, locale = "en_US", message = "No role found"),
      @Localize(type = LanguageTypes.ExceptionsApiCode, locale = "zh_CN", message = "系统无法找到指定的角色"),
  })
