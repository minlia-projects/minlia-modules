...
public class WechatInfoServiceImpl implements WechatInfoService {
    ...
    public void save(WxMpUser wxMpUser) throws WxErrorException {
        WechatOpenAccount wechatOpenAccount = wechatOpenAccountService.findOne(WechatOpenAccountQueryBody.builder().unionId(wxMpUser.getUnionId()).openType(WechatOpenidType.PUBLIC).build());
        ...
        if (null == wechatOpenAccount) {
            ...
            if (CollectionUtils.isNotEmpty(wechatOpenAccounts)) {
                wechatOpenAccount.setUserId(wechatOpenAccounts.get(0).getUserId());
            }
