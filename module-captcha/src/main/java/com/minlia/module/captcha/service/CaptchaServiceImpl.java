...
public class CaptchaServiceImpl implements CaptchaService {
    ...
    @Override
    public Captcha send(CaptchaTO to) {
        Captcha captcha;
        if (to.getMethod().equals(CaptchaMethodEnum.CELLPHONE)) {
            ...
            captcha = this.sendByCellphone(to.getCellphone());
        } else {
            ApiAssert.hasLength(to.getEmail(), CaptchaCode.Message.EMAIL_NOT_NULL);
            captcha = this.sendByEmail(to.getEmail());
        }
        return captcha;
    }
    ...
    public Captcha sendByCellphone(String cellphone) {
        ...
        //查询并判断是否存在
        Captcha captcha = captchaMapper.queryOne(CaptchaQO.builder().cellphone(cellphone).build());
//        ApiPreconditions.is(captcha.getLocked(), 1,"验证码已超出当日发送上线"); TODO
    ...
    public Captcha sendByEmail(String email) {
        //查询并判断是否存在
        Captcha captcha = captchaMapper.queryOne(CaptchaQO.builder().email(email).build());
//        ApiPreconditions.is(captcha.getLocked(), 1,"验证码已超出当日发送上线"); TODO
