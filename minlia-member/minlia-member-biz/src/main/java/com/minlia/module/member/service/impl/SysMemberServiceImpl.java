...
public class SysMemberServiceImpl extends ServiceImpl<SysMemberMapper, SysMemberEntity> implements SysMemberService {
    ...
    @Autowired
    @Qualifier("sysRealName3OldServiceImpl")
    //@Qualifier("sysRealName3ServiceImpl")
    ...
    public Response register(MemberRegisterRo registerRo) {
        Response<SysUserEntity> response = sysUserRegisterService.register(UserRegisterRo.builder().type(SysRegisterTypeEnum.CELLPHONE).areaCode(registerRo.getAreaCode()).cellphone(registerRo.getCellphone()).vcode(registerRo.getVcode()).inviteCode(registerRo.getInviteCode()).roleCode(SysMemberConstants.DEFAULT_ROLE).password(registerRo.getPassword()).build());
    ...
    public Response realName(SysRealNameCro cro) {
        ...
        cro.setCellphone(SecurityContextHolder.getUserContext().getCellphone());
        //判断大于18岁
        ApiAssert.state(IdcardUtil.getAgeByIdCard(cro.getIdNumber()) >= 18, SysRealNameCode.Message.AGE_MUST_BE_OVER18);
        //判断实名次数
        long realNameCount = sysRealNameService.count(Wrappers.<SysRealNameEntity>lambdaQuery().eq(SysRealNameEntity::getUid, cro.getUid()).gt(SysRealNameEntity::getCreateDate, LocalDate.now().atStartOfDay()));
        ApiAssert.state(realNameCount < 3, SysMemberCode.Message.MULTIPLE_FAILURES_PLEASE_WAIT);
        //认证
        ...
        if (realNameResponse.isSuccess()) {
            ...
            //风控 TODO
            RiskKieService.execute(new RiskRealNameEvent(cro.getUid()));
        }
