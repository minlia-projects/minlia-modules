package com.minlia.modules.aliyun.dypls.event;//package com.minlia.modules.starter.sms;
//
//import com.minlia.boot.context.ContextHolder;
//import com.minlia.modules.starter.sms.body.NotificationRequestBody;
//import com.minlia.modules.starter.sms.event.NotificationEvent;
//
///**
// * Created by will on 7/10/17.
// */
//public class SmsNotificationPublisher {
//
//    public static void notification(NotificationRequestBody event) {
//        ContextHolder.getContext().publishEvent(new NotificationEvent(event));
//    }
//
//}
