...
public class SysUserRegisterServiceImpl implements SysUserRegisterService {
    ...
    private Response registerByCellphone(UserRegisterRo to) {
        Response response = captchaService.validity(to.getCellphone(), to.getVcode());
    ...
    public Response availablitity(UserAvailablitityTo body) {
        ...
        if (StringUtils.isNotBlank(body.getUsername()) && sysUserService.count(Wrappers.<SysUserEntity>lambdaQuery().eq(SysUserEntity::getCellphone, body.getCellphone())) > 0) {
