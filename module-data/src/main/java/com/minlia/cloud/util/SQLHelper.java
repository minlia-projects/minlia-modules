...
public class SQLHelper {
    ...
    public static int getCount(final String sql, final Connection connection,
    	...
    	String dbName = FrameworkConfiguration.get("jdbc.type");
...
//                log.debug("COUNT SQL: " + StringUtils.replaceEach(countSql, new String[]{"\n","\t"}, new String[]{" "," "}));
    ...
    public static String generatePageSql(String sql, Page<Object> page, Dialect dialect) {
        if (dialect.supportsLimit()) {
            return dialect.getLimitString(sql, page.getNumber(), page.getNumber()*page.getSize());
        } else {
            return sql;
        }
    }
...
//        return qlString.substring(beginPos);
	...
	public static String removeWhere(String qlString){
        int beginPos = qlString.toLowerCase().indexOf("where");
        if(beginPos>-1) {
            return qlString.substring(beginPos);
        }else{
            return qlString;
