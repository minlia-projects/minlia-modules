...
public class SysArticlePraiseServiceImpl extends ServiceImpl<SysArticlePraiseMapper, SysArticlePraiseEntity> implements SysArticlePraiseService {
    ...
    public SysArticlePraiseServiceImpl(@Lazy SysArticleService sysArticleService) {
    ...
    public boolean praise(Long articleId) {
        ...
        if (isPraised(articleId)) {
            this.remove(Wrappers.<SysArticlePraiseEntity>lambdaQuery().eq(SysArticlePraiseEntity::getArticleId, articleId).eq(SysArticlePraiseEntity::getOperator, SecurityContextHolder.getUid()));
            return false;
        } else {
            this.save(SysArticlePraiseEntity.builder().articleId(articleId).operator(SecurityContextHolder.getUid()).build());
            return true;
        }
    }
    ...
    public boolean isPraised(Long articleId) {
        long countPraise = this.count(Wrappers.<SysArticlePraiseEntity>lambdaQuery().eq(SysArticlePraiseEntity::getArticleId, articleId).eq(SysArticlePraiseEntity::getOperator, SecurityContextHolder.getUid()));
