...
import com.minlia.modules.rbac.config.TokenRedisConstants;
...
public class RbacAuthenticationService extends AbstractAuthenticationService {
    ...
    public Authentication authentication(Authentication authentication) {
        ...
        User user = userMapper.queryByUsernameOrEmailOrCellphone(username,username,username);
        if (null == user) throw new UsernameNotFoundException("User not exists:" + username);
        else if (user.getExpired()) throw new AccountExpiredException("账号过期");
        else if (user.getCredentialsExpired()) throw new CredentialsExpiredException("凭证已过期");
        else if (user.getEnabled()) throw new DisabledException("账号已禁用");
        else if (user.getLocked() && new Date().before(user.getLockTime())) throw new LockedException("账号已锁定");
        else if (!encoder.matches(password,user.getPassword())) {
            //密码错误 锁定次数+1
            user.setLockLimit(user.getLockLimit()+ NumberUtils.INTEGER_ONE);
            //如果超过5次 直接锁定 TODO 动态配置
            if (user.getLockLimit() > 3) {
                user.setLocked(true);
                //错的越多，锁的越久
