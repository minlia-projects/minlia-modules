...
public class ArticleEndpoint {
    ...
    public Response create(@Valid @RequestBody ArticleCRO cro) {
        if (CollectionUtils.isNotEmpty(cro.getCategoryIds())) {
//            cro.setCategoryId(Collections.min(cro.getCategoryIds()));
            cro.setCategoryId(cro.getCategoryIds().get(cro.getCategoryIds().size() -1));
        }
        return Response.success(articleService.create(cro));
    }
    ...
    public Response update(@Valid @RequestBody ArticleURO uro) {
        if (CollectionUtils.isNotEmpty(uro.getCategoryIds())) {
//            uro.setCategoryId(Collections.min(uro.getCategoryIds()));
            uro.setCategoryId(uro.getCategoryIds().get(uro.getCategoryIds().size() -1));
        }
        return Response.success(articleService.update(uro));
    }
    ...
    public Response id(@PathVariable Long id) {
        ...
        if (null != article) {
            ...
            //设置级联categoryIds
            article.setCategoryIds(articleCategoryService.selectCategoryIdsById(article.getCategoryId()));
        }
    ...
    public Response count(@RequestBody ArticleQRO qro) {
        if (CollectionUtils.isNotEmpty(qro.getCategoryIds())) {
//            qro.setCategoryId(Collections.min(qro.getCategoryIds()));
            qro.setCategoryId(qro.getCategoryIds().get(qro.getCategoryIds().size() -1));
        }
        return Response.success(articleService.count(qro));
    }
    ...
    public Response one(@RequestBody ArticleQRO qro) {
        if (CollectionUtils.isNotEmpty(qro.getCategoryIds())) {
//            qro.setCategoryId(Collections.min(qro.getCategoryIds()));
            qro.setCategoryId(qro.getCategoryIds().get(qro.getCategoryIds().size() -1));
        }
        Article article = articleService.oneVO(qro);
        if (null != article) {
            article.setCoverObj(attachmentService.queryOne(AttachmentQRO.builder().relationId(article.getId().toString()).belongsTo(ArticleConstants.ARTICLE_COVER).build()));
